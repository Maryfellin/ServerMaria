...
bool ProcessPacket(int ID, Packet _packettype)
{
	...
	case Message:
	{
		...
		for (int i = 0; i < ClientCount; i++) // Далее нам нужно отправить сообщение каждому пользователю
		{
			...
			{
				cout << "Не удалось отправить сообщение клиента с ID: " << ID << " клиенту с ID: " << i << endl;
			}
		}
		cout << "Сообщение от клиента: " << ID << endl;
		break;
	}

	default:
	{
		cout << "Неизвестный пакет" << _packettype << endl;
		break;
	}
	}
	return true;
...
void ClientHandlerThread(int ID)
{
	...
	cout << "Потеряна связь с клиентом: " << ID << endl;
...
int main()
{
	...
	/*вызываем функцию, проверяем каждый процесс на отладку*/
	dynamic(sListen = socket(AF_INET, SOCK_STREAM, NULL)); // Создаем сокет для прослушивания новых подключений
	dynamic(bind(sListen, (SOCKADDR*)&addr, sizeof(addr))); // Связывание адреса с сокетом
	dynamic(listen(sListen, SOMAXCONN));
